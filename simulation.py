...
class Simulation(object):
    ...
    def _create_population(self, initial_infected):
        ...
        # the correct intial vaccination percentage and initial infected.
                    ...
                    # parameters for person
        # person = Person(a ._id,b .is_alive,c .is_vaccinated,d .infection)
                    # (a)assert person._id == 1
                    # (b)assert person.is_alive == True
                    # (c)assert person.is_vaccinated == True
                    # (d)assert person.infection == None

                    # population_size (1000) * vacc_percentage (50%)
                    # population_size (1000) * initial_infected (10)


        person = Person(0, True, True)
        pop_size = 10
        for i in range(0, pop_size):
            initial_population.append(person)
        # for i in range(len(initial_population)):
        print ([person._id for person in initial_population])
                # print(initial_population[0]._id)
